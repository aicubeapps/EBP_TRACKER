...
│   │                                folded back into this table until now. Both `fd62ff7` and `060350e` are
│   │                                **confirmed live** — the actual deployed source was compared function-by-
│   │                                function against this file and found identical (Section 13.2).
...
```
`daily.today`/`daily.yesterday` deliberately replace the *frontend's* prior client-side today/yesterday derivation (see below) — the old client-side logic used an approximate DST calculation and a single rolling session-start cutoff that could misbucket across weekends/holidays; the new server-side version buckets every row in the already-fetched intraday window by trading-day key and takes the two most recent days that actually have data, which is correct across weekends by construction. 🆕 **2026-08-12 correction**: this was written when the query window was still 48h (this task's original scope); the actual deployed source (Section 13.2) shows commit `fd62ff7` later widened this exact cutoff to **35 days** (`const cutoff = Date.now() - 35 * 24 * 60 * 60 * 1000`, with an in-code comment explaining the daily/weekly arrays both need it), matching Section 2's `/market/breadth` note elsewhere in this report — the "residual limitation" flagged here no longer applies; a weekend/holiday gap would need to exceed 35 days to under-populate `yesterday` now, not 48h.
...
**Verification status — one gap, disclosed rather than fabricated**: `compute-worker` and `worker` were both redeployed and confirmed live (`/health` 200, `market_breadth_cache` queried directly via `wrangler d1 execute --remote`). However, live confirmation that the `'1W_current'` row actually populates, and a full authenticated `GET /market/breadth` response shape check, were **not completed this session** — the forex-weekend gate inside `handleMarketBreadthCron()` (Friday 17:00–Sunday 17:00 NY) was active for the entire session, suppressing every cron cycle before it reaches `computeWeeklyBreadth()`, and no Clerk bearer token was available in-session to call the authenticated route directly. Both are recommended follow-up checks once markets reopen. 🆕 **2026-08-12 — both gaps now closed.** `SELECT tf, computed_at FROM market_breadth_cache` (live, `wrangler d1 execute --remote`) returns three rows — `1H`, `1W`, and `1W_current` — confirming the in-progress weekly row is populating in production. The authenticated `GET /market/breadth` response-shape check was never made as a live HTTP call (still no Clerk bearer token available), but is now closed a stronger way: the actual deployed `worker/src/ebp-worker.js` source was retrieved directly from Cloudflare and read in full (Section 13.2) — its `/market/breadth` handler does query `market_breadth_cache` for both `'1W'` and `'1W_current'` and does return them as `weekly.lastWeek`/`weekly.thisWeek`, exactly as this section describes. (Section 13.2's original claim that `worker` trailed `main` by one commit was itself a false positive, corrected the same session — `worker`'s live deployment matches `main` exactly.)
