...
- **Deployment**: `npx wrangler deploy` per worker, run directly this session for all 6 workers on 2026-08-07 (see Section 10 for the exact commands and verified output); `watchdog-worker` was redeployed twice more on 2026-08-09 (Section 12). Still no CI/CD pipeline in-repo. Frontend via Cloudflare Pages, GitHub-integration auto-build — confirmed 2026-08-07: pushing commit `d20aea3` to `main` triggered a rebuild that picked up the two new frontend env vars; confirmed again 2026-08-09 when the logo/favicon commit (`fd5f951`) was pushed (build success itself not independently re-verified post-push this session — see Section 12). 🆕 **2026-08-12 correction — deployment state does not track `main` for every worker.** `npx wrangler deployments list` (ground truth, read directly this session) shows: `ebp-tracker-worker`'s live deployment is timestamped `2026-08-07T23:25:19Z`, before **two** undeployed commits (`060350e` then `fd62ff7`, both touching `worker/src/ebp-worker.js` — Section 9) — production is running pre-`060350e` code, still true as of this update, not redeployed this session. `compute-worker` shows a live deployment (`2026-08-10T12:15Z`, `Source: Upload`) **after** its last git commit, and is now confirmed (not just suspected) to be running code absent from its committed source (Section 9's `getYahooCandlesFromCache` finding) — also not redeployed/reconciled this session. `watchdog-worker` had the same class of problem (deployed `2026-08-11T12:30Z` from an uncommitted working copy) but **this one is now resolved**: rebuilt, committed (`1c0242a`), and redeployed (`2026-08-12T06:45:17Z`) this session — see Section 14.
...
### 🔶 (2026-08-12) UNTESTED — `watchdog-worker`'s rebuilt breadth/DXY pipeline has not yet been confirmed firing in production
Deployed `2026-08-12T06:45:17Z` (see below and Section 14.2). As of `07:06Z` — past the hourly `:00`-ish window this job fired consistently every prior hour today — no `POST /cron/breadth-fetch` trace of any kind (success or error) appears in `watchdog_log` since `06:01:21Z` (pre-deploy). Not yet diagnosed; see **Section 14.5** for full detail, including a second, separate anomaly (native `scheduled()` heartbeat entries resuming after a ~43-hour gap, contradicting Section 12.3's claim that worker's live cron schedule was deleted). Do not treat the rebuild below as confirmed working end-to-end in production yet.
...
### 🆕 🐛 (2026-08-12) NEW — the same divergence pattern is confirmed on `compute-worker` too, unresolved
While investigating the above, a recovered fragment from a separate, previously-unmerged audit session (branch `claude/ebp-tracker-codebase-audit-o4noyr`, dated 2026-08-10) showed `compute-worker`'s Market Breadth cron reading breadth-pair candles via a function called `getYahooCandlesFromCache()`, sourced from `yahoo_candle_cache`. **Checked this session**: `grep -n "yahoo_candle_cache\|getYahooCandlesFromCache" compute-worker/src/index.js` on current `main` returns **zero matches** — this function does not exist in the committed `compute-worker` source at all. Combined with `compute-worker`'s live deployment (`2026-08-10T12:15:23Z`) postdating its last git commit (`060350e`, `2026-08-08T04:12:32+05:30`), this is the same class of problem `watchdog-worker` had: **`compute-worker`'s live Market Breadth logic is not the code in this repository.** Given my `watchdog-worker` rebuild now writes breadth data into `yahoo_candle_cache` (previously nothing did, per the committed source), it's very likely feeding `compute-worker`'s live (undocumented) breadth logic correctly — but this has not been directly confirmed, and the gap itself is **not fixed**. This was outside the scope of this session's task (only `watchdog-worker` was asked for); flagged here as the next thing that needs the same reconciliation treatment.

### 🆕 (2026-08-12) `worker` (ebp-tracker-worker) production is now confirmed **two** commits behind `main`, not one
Section 13.2 (below) flagged `fd62ff7` as undeployed. A closer look this session found a second, earlier undeployed commit: **`060350e`** (2026-08-08T04:12:32+05:30, "weekly market breadth display + daily strength weekend fix") also touched `worker/src/ebp-worker.js` (+54 lines, predates `fd62ff7`'s own +86 lines to the same file) and `compute-worker/src/index.js` (+31 lines — plausibly related to the `getYahooCandlesFromCache` gap above, not independently confirmed). `worker`'s live deployment is still `2026-08-07T23:25:19Z`, before both. Not redeployed this session (out of scope — only `watchdog-worker` was tasked); still open.
...
### 13.2 — `worker` (`ebp-tracker-worker`): production is behind `main` by **two** commits (🆕 2026-08-12: corrected from "one")

`wrangler deployments list` shows the last deploy at `2026-08-07T23:25:19Z`. `git log -- worker/` shows commit `fd62ff7` ("market breadth historical selectors, refresh animation, header UI fixes") at `2026-08-08T05:07:29+05:30` = `2026-08-07T23:37:29Z` — **12 minutes after** the last deploy, and no deploy has happened since (`wrangler deployments list` documents "the 10 most recent deployments"; this is confirmed the most recent of those 10). `git show fd62ff7 --stat` confirms this commit changed `worker/src/ebp-worker.js` itself (+86/-lines, inside the `/market/breadth` handler — not just frontend), so this is a real backend behavior gap, not a docs-only diff: production's `GET /market/breadth` is still running the pre-`fd62ff7` handler. 🆕 A second look this same day found an **earlier** undeployed commit too: `060350e` ("weekly market breadth display + daily strength weekend fix", `2026-08-08T04:12:32+05:30`, i.e. before `fd62ff7`) also touched `worker/src/ebp-worker.js` (+54 lines) and `compute-worker/src/index.js` (+31 lines) — production is running code from before *both* commits, not just the later one. Since Cloudflare Pages auto-deploys the frontend on every push to `main`, the live frontend almost certainly already expects response shapes both commits added — this is a live frontend/backend mismatch, not just a repo/report one. **Recommended fix: `cd worker && npx wrangler deploy`** (not performed this session — only `watchdog-worker` was in scope for redeployment; flagged for the user to action).

### 13.3 — `compute-worker`: same deployed-after-last-commit pattern as 13.1 — 🆕 (2026-08-12) now CONFIRMED, unresolved

`compute-worker`'s last deploy (`2026-08-10T12:15:23Z`) is also after its last commit (`060350e`, `2026-08-08T04:12:32+05:30`). *Originally written*: "no corroborating evidence... found to confirm the deployed code actually differs" — **superseded later the same day**: a recovered fragment (Section 14) showed `compute-worker`'s live Market Breadth cron reading via `getYahooCandlesFromCache()` from `yahoo_candle_cache`; `grep -n "yahoo_candle_cache\|getYahooCandlesFromCache" compute-worker/src/index.js` against current `main` returns zero matches. This is now a confirmed divergence, the same class as 13.1 was — see Section 9's dedicated entry. Unlike `watchdog-worker`, this one was **not** rebuilt/redeployed this session (out of scope — only `watchdog-worker` was tasked).
...
- **`compute-worker`'s `yahoo_candle_cache` gap**: `grep -n "yahoo_candle_cache\|getYahooCandlesFromCache" compute-worker/src/index.js` against current `main` — zero matches, confirming (not just suspecting, as Section 13.3 originally hedged) the same live/repo divergence `watchdog-worker` had. Recorded in Section 9 and 13.3; **not fixed** — out of scope for this session's `watchdog-worker`-only task.
...
### 14.5 — Addendum: post-deploy freshness confirmation — 🔶 UNTESTED as of this writing, one anomaly found

The pre-deploy baseline (`dxy_candle_cache`/`yahoo_candle_cache` last write `2026-08-12T06:00:56Z`/`06:01:17Z`, `watchdog_log`'s last "Breadth fetch complete" entry `06:01:21Z`) was re-checked repeatedly from `06:47` through `07:06Z` — over 20 minutes past the deploy and past the `07:00` hour boundary this hourly job has fired consistently on (`00:00:34`, `01:01:16`, `02:01:19`, `03:01:16`, `04:01:17`, `05:01:17`, `06:01:21` — one per hour, every hour today before the deploy). **As of `07:06Z`, no `POST /cron/breadth-fetch` invocation trace of any kind exists in `watchdog_log` since `06:01:21Z`** — no success log, no per-symbol error log, nothing — where every prior hour today left one within a minute of the hour. This has not yet been explained: it may be ordinary cron-job.org scheduling jitter that will resolve on its own, or it may indicate the job isn't firing or is failing before any `logWatchdog()` call executes (the route's outer `catch` uses `console.error`, not `logWatchdog`, so a top-level exception in the rebuilt code would be invisible to a D1 query). Manually re-invoking `POST /cron/breadth-fetch` would resolve this immediately but requires the actual `CRON_SECRET` *value*, which this session only ever had the secret *name* for (`wrangler secret list` doesn't expose values). **Flagged as an open, unresolved item — do not read Section 14.2's "deployed successfully" as "confirmed working end-to-end."**

A second, unrelated anomaly surfaced while checking this: `watchdog_log`'s `"Watchdog scheduled tick — heartbeat"` entries (from the native `scheduled()` cron, not the HTTP routes) show a large gap — firing every 15 minutes through `2026-08-10T12:00:59Z`, then **nothing until `2026-08-12T06:46:00Z`** (shortly after this session's deploy), then again at `07:00:43Z`. Section 12.3 documented explicitly deleting `watchdog-worker`'s live Cloudflare-side cron schedule on 2026-08-09 via the Workers API, confirmed empty via a follow-up `GET`; this session's `watchdog-worker/wrangler.toml` still has no `[triggers]` block. Why heartbeat entries resumed immediately after this session's deploy is not established — Section 9/12.2 already documented that a `wrangler.toml` `[triggers]` removal doesn't retroactively clear an already-registered live Cloudflare schedule, so one explanation is that the 2026-08-10 undocumented deploy (Section 13.1/14.1) re-registered a schedule via its own (uncommitted, unknown) `wrangler.toml`, which then sat dormant or logging elsewhere until this session's deploy somehow surfaced it again — but this is speculation, not confirmed. Checking Cloudflare's actual live schedule registration for `ebp-watchdog` (the way Section 12.2 did for `ebp-tracker-worker`) would resolve this but requires a raw Cloudflare API token, which this session deliberately did not extract from `wrangler`'s local credential store. Flagged as a second open item.
