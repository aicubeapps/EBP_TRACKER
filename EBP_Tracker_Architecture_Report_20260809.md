...
### 🐛 (2026-08-12) cron-job.org appears to have stopped calling `watchdog-worker` entirely since the redeploy — likely not a code bug
Deployed `2026-08-12T06:45:17Z` (see below and Section 14.2). A 23-minute background poll (`06:47`–`07:10Z`) found zero `POST /cron/breadth-fetch` trace in `watchdog_log` since `06:01:21Z` (pre-deploy), spanning the `07:00` hourly boundary this job hit within a minute of, every hour, all day before. **Critically, `POST /cron/candle-fetch` — a route this session never modified — also stopped**: last `candle_cache` write `06:31:05Z`, silent through its `06:45`/`07:00` `:15`-min boundaries. `GET /health` stayed `200` throughout. Since the untouched route failed at the same time as the rebuilt one, this points at cron-job.org no longer calling the worker (possibly job auto-disable after a failure during the ~15s deploy window) rather than a bug in the rebuild — but this session has no cron-job.org API key or `CRON_SECRET` value to confirm directly. **Recommended: check both jobs' status/history on the cron-job.org dashboard.** See **Section 14.5** for full detail, including a second, separate anomaly (native `scheduled()` heartbeat entries resuming after a ~43-hour gap, contradicting Section 12.3's claim that the worker's live cron schedule was deleted). Until cron-job.org is checked, treat the rebuild as deployed and code-reviewed but not yet confirmed working end-to-end against real traffic.
